...
import { bullDetail, BullList, OrgList } from '../bull-master-model/bull-master.model';
import { BullMasterService } from '../bull-master.service';
import { NgSelectComponent } from '@ng-select/ng-select';

@Component({
  selector: 'app-bull-master-list',
  templateUrl: './bull-master-list.component.html',
  styleUrls: ['./bull-master-list.component.css']
})
export class BullMasterListComponent implements OnInit {
  ...
  viewOrgDetailsDialog() {
    const dialog = this.dialog.open(ViewOrganizationComponent, {
      data: { orgData: this.orgDetailsByID },
      width: '500px',
      height: '100vh',
...
storeSemenStation(){
  const selectedId = this.searchForm.value.searchValue ? this.searchForm.value.searchValue
                    : this.searchForm.value.bullId 
 const org :any = this.orgsList?.filter(org=>org.subOrgId == selectedId)
  const reqObj = {
   ssId:org && org?.length > 0 ?org[0]?.semenStationCd : null,
   semenName:this.orgDetailsByID?.subOrganizationBasicInfo?.subOrgName ? 
             this.orgDetailsByID?.subOrganizationBasicInfo?.subOrgName
             : this.orgDetailsByID?.subOrgName
  }
  sessionStorage.setItem("semenName",JSON.stringify(reqObj))
}
