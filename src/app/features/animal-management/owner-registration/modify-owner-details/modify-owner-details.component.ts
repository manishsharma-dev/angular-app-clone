...
export class ModifyOwnerDetailsComponent implements OnInit {
  ...
  editOwnerDialog(isView?: boolean) {
    const dialogRef = this.dialog.open(EditOwnerDetailsComponent, {
      data: {
        ...
        redirectLink: '/dashboard/owner/modifyowner',
  ...
  openOtpDialog() {
    const dialogRef = this.dialog.open(OtpDialogComponent, {
      data: {
        ...
        link: '/dashboard/owner/modifyowner',
  ...
  getTehsil(districtCode: any) {
    this.villageList = [];
    // this.professionalInfo.baseLocation.get('tehsilCd').reset();
    // this.professionalInfo.baseLocation.get('villageCd').reset();
    this.isLoadingSpinner = true;
    this.countryService
      .getTehsil(
      ...
      .subscribe(
        (tehsilData: TehsilList[]) => {
          ...
          this.tehsilList = tehsilData.filter(
            (district) =>
              (district.districtCode = (districtCode.target as HTMLInputElement)
                ? +(districtCode.target as HTMLInputElement)?.value
                : districtCode)
          );
        },
      ...
      .subscribe(
        (villageData: VillageList[]) => {
          ...
          this.villageList = villageData?.filter(
            (village) =>
              (village.tehsilCode = (tehsilCode.target as HTMLInputElement)
                ? +(tehsilCode.target as HTMLInputElement)?.value
                : tehsilCode)
          );
        },
