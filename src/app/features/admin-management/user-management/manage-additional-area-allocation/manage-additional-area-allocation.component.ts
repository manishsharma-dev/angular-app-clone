...
@Component({
  selector: 'app-manage-additional-area-allocation',
  templateUrl: './manage-additional-area-allocation.component.html',
  styleUrls: ['./manage-additional-area-allocation.component.css'],
  providers: [TranslatePipe],
})
export class ManageAdditionalAreaAllocationComponent implements OnInit {
  ...
  setStateData() {
    ...
    this.masterData = this.data.stateMaster.map((state) => {
      return {
        ...state,
        section: "state"
      }
    })  // this.userManagementService.getMultiState();
    this.areaAllocation.patchValue({
      selectedState: this.data.selectedData
    });
  ...
  async addStateChild(stateCodeList: number[]) {
    const { selectedDistrict, selectedTehsil, selectedVillage } = this.data.areaAllocation;
    const districtList = await this.userManagementService.getMultiDistricts(stateCodeList).toPromise();
    for (let district of districtList) {
      this.addUniqueItem(selectedDistrict, { ...district, districtCd: district.districtCd }, "district")
    }
    const tehsilList = await this.userManagementService.getMultiTehsils(districtList.map((dist) => dist.districtCd)).toPromise();
    for (let tehsil of tehsilList) {
      this.addUniqueItem(selectedTehsil, { ...tehsil, tehsilCd: tehsil.tehsilCd }, "tehsil")
    }
    const villageList = await this.userManagementService.getMultiVillages(tehsilList.map((teh) => teh.tehsilCd)).toPromise();
    for (let village of villageList) {
      this.addUniqueItem(selectedVillage, { ...village, villageCd: village.villageCd }, "village")
    }
  }

  async addDistrictChild(DistCodeList: number[]) {
    const { selectedDistrict, selectedTehsil, selectedVillage } = this.data.areaAllocation;
    const tehsilList = await this.userManagementService.getMultiTehsils(DistCodeList).toPromise();
    for (let tehsil of tehsilList) {
      this.addUniqueItem(selectedTehsil, { ...tehsil, tehsilCd: tehsil.tehsilCd }, "tehsil")
  ...
  async addTehsilChild(tehsilCodeList: number[]) {
    const { selectedDistrict, selectedTehsil, selectedVillage } = this.data.areaAllocation;
    const villageList = await this.userManagementService.getMultiVillages(tehsilCodeList).toPromise();
    for (let village of villageList) {
      this.addUniqueItem(selectedVillage, { ...village, villageCd: village.villageCd }, "village")
  ...
  addUniqueItem(selectedArray: Area[], newItem: Area, section?: string) {
    switch (section) {
      case "state":
        break;
      case "district":
  ...
  async removeState(i) {
    const value = this.formControls.selectedState.value;
    const selectedState = value[i];
    const villageCount = this.data.areaAllocation.selectedVillage.reduce((accumulator, object) => {
      if (object.stateCd === value[i].stateCd) {
        accumulator += 1;
      }
      return accumulator;
    }, 0);
    ...
    if (val === false) return;
    if (this.stateSelected == value[i].stateCd) {
  ...
  async onAreaRemove(areas: any, control: string, section: string) {
    ...
    const removeDistrict = async (districtMaster) => {
      ...
      const villageCount = districtMaster.reduce((acc, dist) => {
        acc += villageMaster.filter((area) => area.districtCd === dist.districtCd)?.length;
        return acc;
      }, 0);

      if (villageCount && (await showConfirmationDialog(`Are you sure you want to delete this district, ${villageCount} village(s) will be deleted.`))) {
      ...
      for (let dist of districtMaster) {
        const index = this.data.areaAllocation.selectedDistrict.findIndex((temp) => temp.districtCd == dist.districtCd);
        this.data.areaAllocation.selectedDistrict.splice(index, 1);
        this.removeDistrictChild(dist);
      }
    };
    ...
    const removeTehsil = async (tehsilMaster) => {
      ...
      const villageCount = tehsilMaster.reduce((acc, dist) => {
        acc += villageMaster.filter((area) => area.tehsilCd === dist.tehsilCd)?.length;
        return acc;
      }, 0);

      if (villageCount && (await showConfirmationDialog(`Are you sure you want to delete this tehsil, ${villageCount} village(s) will be deleted.`))) {
      ...
      for (let dist of tehsilMaster) {
        const index = this.data.areaAllocation.selectedTehsil.findIndex((temp) => temp.tehsilCd == dist.tehsilCd);
        this.data.areaAllocation.selectedTehsil.splice(index, 1);
        this.removeTehsilChild(dist);
      }
    };
        ...
        switch (control) {
          case AreaType.State:
            if (!areas?.value?.stateCode) {
              ...
              const villageCount = this.data.areaAllocation.selectedVillage.reduce((accumulator, object) => {
                if (object.stateCd === areas?.value?.stateCode) {
                  accumulator += 1;
                }
                return accumulator;
              }, 0);
          ...
          case AreaType.District:
            if (!areas?.value?.districtCd) {
              ...
              const villageDCount = this.data.areaAllocation.selectedVillage.reduce((accumulator, object) => {
                if (object.districtCd === areas?.value?.districtCd) {
                  accumulator += 1;
                }
                return accumulator;
              }, 0);

              if (villageDCount && (await showConfirmationDialog(`Are you sure you want to delete this district, ${villageDCount} village(s) will be deleted.`))) {
                districtControl.setValue([...districtControl?.value, areas?.value]);
                return;
              }

              if (this.districtSelected == areas.value.districtCd) {
                this.districtSelected = null;
                this.tehsilSelected = null;
              }

              const isExist = this.data.areaAllocation.selectedDistrict.findIndex((dist) => dist.districtCd == areas.value.districtCd);
              if (isExist != -1) {
                this.data.areaAllocation.selectedDistrict.splice(isExist, 1);
              }
          ...
          case AreaType.Tehsil:
            if (!areas?.value?.tehsilCd) {
              ...
              const villageTCount = this.data.areaAllocation.selectedVillage.reduce((accumulator, object) => {
                if (object.tehsilCd === areas?.value?.tehsilCd) {
                  accumulator += 1;
                }
                return accumulator;
              }, 0);

              if (villageTCount && (await showConfirmationDialog(`Are you sure you want to delete this tehsil, ${villageTCount} village(s) will be deleted.`))) {
                tehsilControl.setValue([...tehsilControl.value, areas?.value]);
                return;
              }

              if (this.tehsilSelected == areas.value.tehsilCd) {
                this.tehsilSelected = null;
              }

              const isExistTehsil = this.data.areaAllocation.selectedTehsil.findIndex((dist) => dist.tehsilCd == areas.value.tehsilCd);
              if (isExistTehsil != -1) {
                this.data.areaAllocation.selectedTehsil.splice(isExistTehsil, 1);
              }
