...
@Component({
  selector: 'app-without-camp-details',
  templateUrl: './without-camp-details.component.html',
  styleUrls: ['./without-camp-details.component.css'],
  providers: [TranslatePipe]
})
export class WithoutCampDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogReference: MatDialogRef<WithoutCampDetailsComponent>,
  ...
  ngOnInit(): void {
    ...
    // Vaccination Type
    this.vaccinationService.getVaccinationType('vaccination_type').subscribe((res: any[]) => {
      this.getVaccinationType = res;
    }, (error) => {
    });
    // getRepeatVaccinationReason Type
    this.vaccinationService.getRepeatVaccinationReason('repeat_vaccination_reason').subscribe((res: any[]) => {
  ...
  withoutCampaignVaccDetails(data) {
    ...
    let dataWithoutCampaign = this.data.selectedtagIdWithOutCampaign;
  ...
  vaccinationDateWithout() {
    this.vaccinationService.getWithoutVaccinationDate('vaccinationDate').subscribe((res: any) => {
      if (res) {
  ...
  get f() {
    return this.vaccinationDetailsWithoutCampaign.controls;
  }
  ...
  uploadImageWithout(event, index) {
    ...
    let WithoutCampaignTagId = this.vaccinationDetailsWithoutCampaign['controls']
  ...
  createRequestForSubmit(request) {
    request['updateLastVaccAndDewDateReq'] = {
      vaccinationDeWormerDate: this.today,
      animalId: this.data2.data.map(a => a.animalId),
      vaccinationDewormingFlag: 'V',
      searchByFlag: "NA"

    }
    request.selected_tagId_details && request.selected_tagId_details.forEach((vacc, index) => {
      ...
      if (vacc['vaccinationType'] == 5) {
        vacc['repeatVaccinationReasonCd'] = vacc['repeatVaccinationReasonCd'].toString();
  ...
  removeRow(i: number) {
    (
      this.vaccinationDetailsWithoutCampaign.get('selected_tagId_details') as FormArray
    ).removeAt(i);
